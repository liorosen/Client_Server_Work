...
//        String new_line_clearer = new String(packet);
//        new_line_clearer = new_line_clearer.replace('\n','\r');
//        packet = new_line_clearer.getBytes();
        return packet;
    }
...
//    }
    ...
    private void sendFile(byte[] data) {
        ...
        while (offset < data.length) {
            ...
            System.out.println(createDataPacket(blockNumber, blockData).length);
...
//        }
    ...
    private byte[] uploadFiletoMemory(String fileName) {
        try {
            Scanner fileReader = new Scanner( new FileReader(FILES_DIRECTORY  + fileName));
